...
nonisolated final class MagicCutsUITests: XCTestCase {
    ...
    @MainActor func testPhysicalDiscoveryReceivesAdvertisements() throws {
        ...
        app.launch()
        XCTAssertTrue(app.buttons["Find a device"].waitForExistence(timeout: 10))
        app.buttons["Find a device"].tap()
    ...
    @MainActor func testExistingUserSkipsWelcomeAndResumesEvidence() {
        ...
        app.staticTexts["Desk sensor"].firstMatch.tap()
    ...
    @MainActor func testLargeTextAndAccessibility() throws {
        ...
        try app.performAccessibilityAudit(for: [.contrast, .hitRegion, .sufficientElementDescription, .trait])
        app.staticTexts["Desk sensor"].firstMatch.tap()
